# -*- coding: utf-8 -*-
"""
Yearly Actual Evapotranspiration from WaPOR v3 (L1-AETI-A / L2-AETI-A)

This is a v3 replacement of the original v2-based AET_yearly module.
...
def main(Dir,
         ...
         Waitbar=1):
    """
    Download yearly WaPOR v3 Actual ET (AETI) for given period and bbox.
    ...
    level : int
        1 for L1-AETI-A (continental), 2 for L2-AETI-A (national)
    version : int
    ...
    if level == 1:
        mapset_code = 'L1-AETI-A'
    elif level == 2:
        mapset_code = 'L2-AETI-A'
    else:
        print('This module only supports level 1 and level 2 data.')
        return None

    print(f"\nDownload yearly WaPOR v3 Actual ET (AETI) data ({mapset_code}) "
    ...
    for dt, code, url in selected:
        fname = 'AETI_WAPOR.v3_level{}_mm-year-1_annually_{:04d}.tif'.format(
            level, dt.year
        )
    ...
    print(f"\nFinished downloading WaPOR v3 yearly Actual ET ({mapset_code})")
    return out_dir