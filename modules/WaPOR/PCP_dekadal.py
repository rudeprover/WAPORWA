# -*- coding: utf-8 -*-
"""
Dekadal precipitation from WaPOR v3 (L1-PCP-D / L2-PCP-D)

This is a v3 replacement of the original v2-based PCP_dekadal module.
Note: Dekadal values are already integrated over the dekad period.
"""
...
def _parse_date_from_code(code):
    ...
    # Handle dekadal format (YYYY-MM-D1, YYYY-MM-D2, YYYY-MM-D3)
    if 'D' in token[-2:]:
        token = token[:-3]  # Remove -D1, -D2, or -D3
...
def main(Dir,
         ...
         Waitbar=1):
    """
    Download dekadal WaPOR v3 precipitation for given period and bbox.
    ...
    level : int
        1 for L1-PCP-D (continental), 2 for L2-PCP-D (national)
    version : int
    ...
    if level == 1:
        mapset_code = 'L1-PCP-D'
    elif level == 2:
        mapset_code = 'L2-PCP-D'
    else:
        print('This module only supports level 1 and level 2 data.')
        return None

    print(f"\nDownload dekadal WaPOR v3 precipitation data ({mapset_code}) "
    ...
    for dt, code, url in selected:
        # Extract raster ID for filename
    ...
    print(f"\nFinished downloading WaPOR v3 dekadal precipitation ({mapset_code})")
    return out_dir