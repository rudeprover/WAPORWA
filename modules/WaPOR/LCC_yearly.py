# -*- coding: utf-8 -*-
"""
Yearly Land Cover Classification from WaPOR v3 (L1-LCC-A / L2-LCC-A)

This is a v3 replacement of the original v2-based LCC_yearly module.
Note: LCC is categorical data, no scale factor is applied.
"""
...
SCALE_FACTOR = 1.0  # No scaling for categorical land cover data
...
def main(Dir,
         ...
         Waitbar=1):
    """
    Download yearly WaPOR v3 Land Cover Classification for given period and bbox.
    ...
    level : int
        1 for L1-LCC-A (continental), 2 for L2-LCC-A (national)
    version : int
    ...
    if level == 1:
        mapset_code = 'L1-LCC-A'
    elif level == 2:
        mapset_code = 'L2-LCC-A'
    else:
        print('This module only supports level 1 and level 2 data.')
        return None

    print(f"\nDownload yearly WaPOR v3 Land Cover Classification data ({mapset_code}) "
    ...
    for dt, code, url in selected:
        fname = 'LCC_WAPOR.v3_level{}_annually_{:04d}.tif'.format(
            level, dt.year
        )
        ...
        # Read and save (no scaling for categorical data)
        driver, NDV, xsize, ysize, GeoT, Projection = gis.GetGeoInfo(tmp_path)
        arr = gis.OpenAsArray(tmp_path, nan_values=False)

        # For LCC, preserve integer values - no scaling needed
        gis.CreateGeoTiff(out_path, arr.astype("int16"),
    ...
    print(f"\nFinished downloading WaPOR v3 yearly Land Cover Classification ({mapset_code})")
    return out_dir